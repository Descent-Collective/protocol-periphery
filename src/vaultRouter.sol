...
//  ==========  External imports    ==========
import {ERC20Permit as ERC20} from "@openzeppelin/contracts/token/ERC20/extensions/ERC20Permit.sol";
...
contract VaultRouter is AccessControl {
        ...
        unchecked {
            ...
            for (uint256 i; i < _encodedParameters.length; ++i) {
                ...
                if (Operations(_operation) == Operations.Invalid) {
                    ...
                    _requireInputIsRightLength(_encodedParameters[i], 64);
                    (ERC20 _token, uint256 _amount) = abi.decode(_encodedParameters[i], (ERC20, uint256));
    ...
    function approveTokenForVault(address _token, address _vaultAddress, bool isMax)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        SafeERC20.forceApprove(ERC20(_token), _vaultAddress, isMax ? type(uint256).max : 0);
    }
