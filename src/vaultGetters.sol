...
contract VaultGetters {
    ...
    function _getVaultMapping(
        Vault _vaultContract,
        ERC20 _collateralToken,
        address _owner
    ) private view returns (IVault.VaultInfo memory) {
        (
    ...
    function _getCollateralMapping(
        Vault _vaultContract,
        ERC20 _collateralToken
    ) private view returns (IVault.CollateralInfo memory) {
        (
    ...
    function checkHealthFactor(
        ...
        IVault.VaultInfo memory _vault = _getVaultMapping(
            _vaultContract,
            _collateralToken,
            _owner
        );
        IVault.CollateralInfo memory _collateral = _getCollateralMapping(
            _vaultContract,
            _collateralToken
        );
        ...
        (uint256 _currentAccruedFees, ) = _calculateAccruedFees(
            _vaultContract,
            _vault,
            _collateral
        );
        ...
        uint256 _collateralValueInCurrency = _getCurrencyValueOfCollateral(
            _vault,
            _collateral
        );

        uint256 _adjustedCollateralValueInCurrency = (_collateralValueInCurrency *
                _collateral.liquidationThreshold) / PRECISION;

        return
            (_adjustedCollateralValueInCurrency * PRECISION) / _totalUserDebt;
    }
    ...
    function getMaxBorrowable(
        ...
        return
            int256(_adjustedCollateralValueInCurrency) -
            int256(_borrowedAmount);
    }
    ...
    function getMaxWithdrawable(
        ...
        uint256 _collateralAmountFromCurrencyValue = _getCollateralAmountFromCurrencyValue(
                _collateral,
                _borrowedAmount
            );

        // adjust for liquidation ratio
        uint256 _adjustedCollateralAmountFromCurrencyValue = (_collateralAmountFromCurrencyValue *
                PRECISION) / _collateral.liquidationThreshold;

        // return diff in depoisted and expected collaeral bal
        return
            int256(_vault.depositedCollateral) -
            int256(_adjustedCollateralAmountFromCurrencyValue);
    }
    ...
    function getVault(
        ...
        return (
            _vault.depositedCollateral,
            _vault.borrowedAmount,
            _accruedFees
        );
    }
    ...
    function _getCurrencyValueOfCollateral(
        IVault.VaultInfo memory _vault,
        IVault.CollateralInfo memory _collateral
    ) internal pure returns (uint256) {
        uint256 _currencyValueOfCollateral = (_scaleCollateralToExpectedPrecision(
                _collateral,
                _vault.depositedCollateral
            ) *
                _collateral.price *
    ...
    function _getCollateralAmountFromCurrencyValue(
        IVault.CollateralInfo memory _collateral,
        uint256 _amount
    ) internal pure returns (uint256) {
        uint256 _collateralAmountOfCurrencyValue = (_scaleCollateralToExpectedPrecision(
                _collateral,
                _amount
            ) * PRECISION) / (_collateral.price * ADDITIONAL_FEED_PRECISION);
    ...
    function _calculateAccruedFees(
        ...
        uint256 _totalCurrentAccumulatedRate = _calculateCurrentTotalAccumulatedRate(
                _vaultContract,
                _collateral
            );

        uint256 _accruedFees = ((_totalCurrentAccumulatedRate -
            _vault.lastTotalAccumulatedRate) * _vault.borrowedAmount) /
            PRECISION;

        return (_accruedFees, _totalCurrentAccumulatedRate);
    }

    function _calculateCurrentTotalAccumulatedRate(
        Vault _vaultContract,
        IVault.CollateralInfo memory _collateral
